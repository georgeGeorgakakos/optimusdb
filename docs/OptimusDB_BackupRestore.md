...
6. **Stream archive**: `manifest.json` → `sqlite/optimusdb.db` → one `orbitdb/{store}.jsonl` per store, all wrapped in a `tar.gz` that streams directly into the HTTP response body. Nothing is buffered in memory except the per-store document list.
7. **Client** receives `application/gzip` with a `Content-Disposition: attachment` header. The filename follows the pattern `optimusdb-export-{UTC-timestamp}.tar.gz`.
